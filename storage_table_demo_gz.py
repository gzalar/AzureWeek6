...
resourcegroup_name = 'gz'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'gz'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
if response.status_code == 200 or response.status_code == 201:
    ...
    print('Error creating resource group ' + resourcegroup_name + ' ' + str(response.status_code))
...
if response.status_code == 202:
    ...
    print('Error creating storage account ' + storageaccount_name + ' ' + str(response.status_code))
...
# Here, we add an entry for first car with two pieces of data - the name, and the price
#
...
print('Created entry for ' + car.make + ' ' + car.model + '\n')
...
# Although the actual price is minimal (fractions of a cent per month) for the three entities we created, it's good to clean up resources when you're done
###
